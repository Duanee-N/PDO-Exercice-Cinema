...
class CinemaController{
    ...
    public function detailFilms($id){ 
        ...
        $requete1=$pdo->prepare("SELECT id_film, titre_film, annee_sortie_fr, TIME_FORMAT(SEC_TO_TIME(duree * 60), '%HH%i') AS duree, synopsis, note, affiche, r.id_realisateur, prenom, nom, f.likes
        FROM film f
        INNER JOIN realisateur r ON r.id_realisateur = f.id_realisateur
        INNER JOIN personne p ON p.id_personne = r.id_personne
    ...
    public function detailLike($id){ 
        ...
        $requeteLike=$pdo->prepare("UPDATE film f
        SET f.likes = f.likes + 1
        WHERE f.id_film = :id"); 
        $requeteLike->execute(["id"=>$id]);
        header("Location:index.php?action=detailFilms&id=".$_GET['id']);die;
    }

    public function detailDislike($id){ 
        $pdo=Connect::seConnecter(); 

        $requeteDislike=$pdo->prepare("UPDATE film f
        SET f.likes = f.likes - 1
        WHERE f.id_film = :id"); 
        $requeteDislike->execute(["id"=>$id]);
        header("Location:index.php?action=detailFilms&id=".$_GET['id']);die;
    }
    ...
    public function detailActeurs($id){ 
        $pdo=Connect::seConnecter(); 
        $requete=$pdo->prepare("SELECT f.id_film, titre_film, annee_sortie_fr, prenom, nom, r.id_role, nom_role, date_naissance, sexe /* portrait */
        FROM casting c
        INNER JOIN film f ON f.id_film = c.id_film
        INNER JOIN acteur a ON a.id_acteur = c.id_acteur
        INNER JOIN personne p ON p.id_personne = a.id_personne
